...
Module variables
    ...
    Public p_TypeExecution As String
    ...
    Public p_xmlSoapLeanTest As New XMLSoapLeanTest
    Public p_APIIRestLeanTest As New APIRestLeanTest
    ...
    Public Class LibGlobal
        ...
        Public Shared Sub LogExecution(MethodName As String, State As StateLog, Optional isTestlog As Boolean = False)
            Try
                ...
                End Select
                Console.WriteLine("*************************************************************************************************************************")
                Console.WriteLine("*********************************************LOG EXECUTION LEANTEST AUTOMATION*******************************************")
        ...
        Function Open(url As String) As Boolean
            ...
            Try
                ...
                Select Case p_ToolName
                    ...
                    '    librarySeleniumRCInteraction.Open(url)
                    'Case ActiveTool.SeleniumWDBrowserStack
                    '    librarySeleniumWDBSInteraction.Open(url)
                    Case ActiveTool.SeleniumWD
        ...
        Function OpenWindow(url As String, IDWindow As String) As Boolean
            ...
            Try
                Select Case p_ToolName
                    'Case ActiveTool.SeleniumRC
                    '    librarySeleniumRCInteraction.OpenWindow(url, IDWindow)
                    'Case ActiveTool.SeleniumWDBrowserStack
                    '    librarySeleniumWDBSInteraction.OpenWindow(url, IDWindow)
                    Case ActiveTool.Appium
        ...
        Function WaitForPageToLoad(Optional TimeOut As Integer = 30000) As Boolean
            ...
            Try
                Select Case p_ToolName
                    'Case ActiveTool.SeleniumRC
                    '    librarySeleniumRCInteraction.WaitForPageToLoad(TimeOut)
                    'Case ActiveTool.SeleniumWDBrowserStack
                    '    librarySeleniumWDBSInteraction.WaitForPageToLoad(TimeOut)
                    Case ActiveTool.SeleniumWD
        ...
        '    End Try
        'End Function
        Private Function Type(ImgPathOrCoordinatesXY As String, value As String, waitElement As Boolean) As String
            Try
                Dim dv = New Devices
                If waitElement Then
                    If Not WaitExist(ImgPathOrCoordinatesXY, typeIdentification.leanTest) Then Throw New Exception("Element not found: " & ImgPathOrCoordinatesXY)
                End If
                If Not pc_dv.Type(ImgPathOrCoordinatesXY, value, p_speedyExecution) Then
        ...
        Function [Set](elementOrImgPathOrCoordinatesXY As String, value As String, Optional fieldOut As String = Nothing, Optional typeIdentification As typeIdentification = typeIdentification.xpath) As Boolean
            ...
            Try
                If typeIdentification = LibGlobal.typeIdentification.leanTest Then
                    Type(elementOrImgPathOrCoordinatesXY, value, True)
                Else
                    Select Case p_ToolName
                        Case ActiveTool.SeleniumWD
                ...
                If Not String.IsNullOrEmpty(fieldOut) And Not String.IsNullOrEmpty(value) And typeIdentification <> LibGlobal.typeIdentification.leanTest Then
                    Test.GetValueAndSave(elementOrImgPathOrCoordinatesXY, fieldOut, typeIdentification, True)
                ...
                ElseIf typeIdentification = LibGlobal.typeIdentification.leanTest Then
                    If Not String.IsNullOrEmpty(fieldOut) Then Call SetValueOutput(fieldOut, value)
                End If
        ...
        Function IsEditable(element As String) As Boolean
            ...
            End Try
        End Function
        Private Function SelectLean(ImgPathOrCoordinatesXY As String, value As String, indexItem As Integer, waitElement As Boolean)
            Try
                Dim dv = New Devices
                If waitElement Then
                    If Not WaitExist(ImgPathOrCoordinatesXY, typeIdentification.leanTest) Then Throw New Exception("Element not found: " & ImgPathOrCoordinatesXY)
                End If
                If Not pc_dv.SelectValue(ImgPathOrCoordinatesXY, value, indexItem, p_speedyExecution) Then
                    Throw New Exception("Element not found: " & ImgPathOrCoordinatesXY)
                Else
                    Return True
                End If
        ...
        Function [Select](elementOrImgPathOrCoordinatesXY As String, value As String, Optional typeIdentification As typeIdentification = typeIdentification.xpath, Optional indexItem As Integer = 1) As String
            ...
            Try
                If typeIdentification = LibGlobal.typeIdentification.leanTest Then
                    SelectLean(elementOrImgPathOrCoordinatesXY, value, indexItem, True)
                Else
                    Select Case p_ToolName
                        'Case ActiveTool.SeleniumRC
        ...
        Public Function GetPositionByImage(pathImage2 As String, Optional acceptancePercentage As String = "1") As String
            ...
            End Try
        End Function
        Public Function GetText(Optional ImgPathOrCoordinatesXY As String = Nothing)
            Try
                Dim dv = New Devices
        ...
        Function GetSelectedValueAndSave(element As String, field As String, Optional typeIdentification As typeIdentification = typeIdentification.xpath) As String
            ...
            End Try
        End Function
        Function GetValueAndSave(ImgPathOrCoordinatesXY As String, field As String, Optional typeIdentification As typeIdentification = typeIdentification.xpath, Optional isType As Boolean = False) As String
            Dim msg As String
        ...
        Public Sub MouseClick(x As Integer, y As Integer, Optional value As Boolean = True)
            ...
            dv.MouseClick(x, y)
        End Sub
        Public Sub MouseDoubleClick(x As Integer, y As Integer, Optional value As Boolean = True)
            If Not value Then Exit Sub
            Wait(p_speedyExecution)
            Dim dv = New Devices
            Dim p = New MousePositions
        ...
        End Function
        Public Function TestLog(stepName As String, ExpectedResults As String, ActualResults As String, TypeLog As typelog, Optional Check As String = Nothing, Optional evidenceVisual As Boolean = True) As Boolean
        ...
        Public Function CheckPointTest(checkpoint As String, expectedResults As String, Optional field_out As String = Nothing, Optional IsTestLog As Boolean = True) As String
            If p_TypeExecution = "Performance Test" Then Return Nothing 'teste de performance nao tem checkpoint
            ...
            Dim element1 As String = Nothing
            Dim element2 As String = Nothing
            Dim valueTest As String = Nothing
            Dim vals() As String = Split(checkpoint, ";")
            Dim statusTest As String = Nothing
            Dim messageReturn As String = Nothing
            Dim isEvidenceVisual As Boolean = True
            ...
            Try
                Try
                    ...
                    element1 = elements(0)
                    If UBound(elements) > 1 Then element2 = elements(1)
                    valueTest = vals(2)
                Catch ex As Exception
                End Try

                Select Case TypeVerify
                    Case "Existing Image"
                        If Test.WaitExist(element1, typeIdentification.leanTest) Then
                            statusTest = "Passed"
                            messageReturn = "A imagem esperada foi apresentada com sucesso!"
                        Else
                            statusTest = "Failed"
                            messageReturn = "A imagem esperada não foi apresentada durante o tempo de timeout"
                        End If
					
                    Case "Text in the field (field_out in datatest)"
                        Dim elementLocal As String = Test.GetValueOutput(p_IDTest, p_TableTest, element1)
                        If InStr(elementLocal, valueTest) > 0 Then
                            statusTest = "Passed"
                            messageReturn = "O texto [" & valueTest & "] contêm no texto [" & elementLocal & "] armazenado em [" & element1 & "]"
                        Else
                            statusTest = "Failed"
                            messageReturn = "O texto [" & valueTest & "] não contêm no texto [" & elementLocal & "] armazenado em [" & element1 & "]"
                        End If
						isEvidenceVisual=False
                    Case "Text in the field (datatest) equal to a field (field_out in datatest)"
                        Dim elementLocal1 As String = Test.GetValueOutput(p_IDTest, p_TableTest, element1)
                        Dim elementLocal2 As String = Test.GetValueOutput(p_IDTest, p_TableTest, element2)
                        If InStr(elementLocal1, elementLocal2) > 0 Then
                            statusTest = "Passed"
                            messageReturn = "O texto [" & elementLocal1 & "] armazenado no campo [" & element1 & "] contêm no texto [" & elementLocal1 & "] armazenado em [" & element2 & "]"
                        Else
                            statusTest = "Failed"
                            messageReturn = "O texto [" & elementLocal1 & "] armazenado no campo [" & element1 & "] não contêm no texto [" & elementLocal1 & "] armazenado em [" & element2 & "]"
                        End If
						isEvidenceVisual=False
                    Case "Text in the page (HTML)"
                        Dim html As String = Nothing
                        For i = 0 To p_timeout
                            html = Test.GetHtmlSource
                            If Not String.IsNullOrEmpty(html) Then Exit For
                            Test.Wait(1000)
                        Next
                        If InStr(html, valueTest) > 0 Then
                            statusTest = "Passed"
                            messageReturn = "O texto [" & valueTest & "] contêm na página"
                        Else
                            statusTest = "Failed"
                            messageReturn = "O texto [" & valueTest & "] não contêm na página"
                        End If
						
                    Case "Text in the page (HTML) equal to a field (datatest)"
                        Dim html As String = Nothing
                        For i = 0 To p_timeout
                            html = Test.GetHtmlSource
                            If Not String.IsNullOrEmpty(html) Then Exit For
                        ...
                        Next
                        Dim elementLocal As String = Test.GetValueOutput(p_IDTest, p_TableTest, element1)
                        If InStr(html, elementLocal) > 0 Then
                            statusTest = "Passed"
                            messageReturn = "O texto [" & elementLocal & "] armazenado em [" & element1 & "] contêm na página"
                        Else
                            statusTest = "Failed"
                            messageReturn = "O texto [" & elementLocal & "] armazenado em [" & element1 & "] não contêm na página"
                        End If
                    Case "Text in the field (field_out in datatest) is different from nothing"
                        Dim elementLocal As String = Test.GetValueOutput(p_IDTest, p_TableTest, element1)
                        If Not String.IsNullOrEmpty(elementLocal) Then
                            statusTest = "Passed"
                            messageReturn = "O texto [" & valueTest & "] não contêm no texto [" & elementLocal & "] armazenado em [" & element1 & "]"
                        Else
                            statusTest = "Failed"
                ...
                If p_ToolName = ActiveTool.SeleniumWD Then
                    ...
                    End If
                End If
				If Not String.IsNullOrEmpty(TypeVerify) then
	                If statusTest = "Passed" Then
	                    Test.TestLog("CheckPoint", "Resultado esperado: " & expectedResults, "Passo executado com sucesso! " & messageReturn, typelog.Passed, , isEvidenceVisual)
	                Else
	                    Test.TestLog("Checkpoint", "Resultado esperado: " & expectedResults, "Passo executado com falha! Menssagem: " & messageReturn, typelog.Failed, , isEvidenceVisual)
	                End If
                End If
        ...
        Function ClipBoardGetText() As String
            ...
            End Try
        End Function
        Function ClipBoarContainsText() As Boolean
            Try
        ...
        Function RemoveNullCharacters(Text) As String
            ...
            End Try
        End Function
        Function ExistProcess(ProcessName As String) As Boolean
            Try
                Dim util = New Utilities
        ...
        Function FileCompare(file1 As String, file2 As String, fileOutput As String, separator As String) As String
            Try
                LogExecution("FileCompare", StateLog.Start)
                Dim file = New FileTexts
                file.FileCompare(file1, file2, fileOutput, separator)
                LogExecution("FileCompare", StateLog.Finish)
                Return True
            Catch ex As Exception
                Return Nothing
            End Try
        ...
        Function XMLSetValue(Node As String, pathXML As String)
            ...
            End Try
        End Function
        Function XMLSetValue(element As String, value As String, pathXML As String)
            Try
                LogExecution("XMLSetValue", StateLog.Start)
                Dim xml = New XMLFile
                xml.CreateElement(element, value, pathXML)
                LogExecution("XMLSetValue", StateLog.Finish)
                Return True
            Catch ex As Exception
                Return False
            End Try
        ...
        Function ValidadeFileField(pathFile As String, settings As String, fieldName As String, defaultValue As String) As String
            Try
                Dim file = New FileTexts
                Dim returnValues As String = file.ValidadeFileField(pathFile, settings, fieldName, defaultValue)
                LogEvidencesFileTest(returnValues, fieldName)
                Return Nothing
            Catch ex As Exception
                Throw New Exception(ex.Message)
            End Try
        ...
        Function ValidadeDBFField(pathFile As String, settings As String, fieldName As String, defaultValue As String) As String
            Try
                Dim dbf = New DBFAdapter
                Dim returnValues As String = dbf.ValidadeDBFField(pathFile, settings, fieldName, defaultValue)
                LogEvidencesFileTest(returnValues, fieldName)
                Return Nothing
            Catch ex As Exception
                Throw New Exception(ex.Message)
        ...
        Private Sub LogEvidencesFileTest(returnValue As String, fieldName As String)
            Try
                Dim messages As String() = returnValue.Split(vbCrLf)
                For i = 0 To (UBound(messages))
                    Dim message As String() = messages(i).Split(";")
                    Dim resultType As String = message(0)
                    Dim resultMessage As String = message(1)

                    If resultType = "Failed" Then
                        Test.TestLog("Validar os valores do campo " & fieldName, "Regras de layout validadas", resultMessage, typelog.Failed, "", False)
                    Else
                        Test.TestLog("Validar os valores do campo " & fieldName, "Regras de layout validadas", resultMessage, typelog.Passed, "", False)
                    End If
                Next
            Catch ex As Exception
        ...
        End Sub
        'call webservice**********************************************************************************************************************************
        Function SetValueXMLElement(element As String, value As String, fieldOut As String, envelope As String) As String
            Try
                If Mid(value, 1, 1) = "@" Then
                    LogExecution("Set.HandlerValue value: " & value, StateLog.Information)
                    value = HandlerValue(value)
                End If
                LogExecution("SetValueXMLElement", StateLog.Start)
                Dim xml = New XMLFile
                If Mid(value, 1, 1) = "@" Then
                    LogExecution("Set.HandlerValue value: " & value, StateLog.Information)
                    value = HandlerValue(value)
                    If Not String.IsNullOrEmpty(fieldOut) Then Call SetValueOutput(fieldOut, value)
                End If
                LogExecution("SetValueXMLElement", StateLog.Finish)
                Return xml.SetValueXMLElement(element, value, envelope)
            Catch ex As Exception
                Return Nothing
            End Try
        End Function

        Public Function CallWebService(uri As String, envelopeXml As String, parametersXML As String, typeExecution As String) As XMLSoapLeanTest
            Try
                Dim xml = New XMLFile
                Dim xmlSoapLeanTest = New XMLSoapLeanTest
                LogExecution("Url: " & uri, StateLog.Start)
                LogExecution("Sends Request: " & envelopeXml, StateLog.Information)
                xmlSoapLeanTest = xml.CallWebService(uri, envelopeXml, parametersXML, p_IDTest, p_TableTest, typeExecution)
                LogExecution("Get Response: " & xmlSoapLeanTest.InnerXML, StateLog.Finish)
                Return xmlSoapLeanTest
            Catch ex As Exception
                Return Nothing
            End Try
        End Function
        'call api rest**********************************************************************************************************************************
        Function SetValueJson(element As String, value As String, fieldOut As String, envelope As String) As String
            Try
                If Mid(value, 1, 1) = "@" Then
                    LogExecution("Set.HandlerValue value: " & value, StateLog.Information)
                    value = HandlerValue(value)
                End If
                LogExecution("SetValueJson", StateLog.Start)
                Dim apiRest = New APIRest
                If Mid(value, 1, 1) = "@" Then
                    LogExecution("Set.HandlerValue value: " & value, StateLog.Information)
                    value = HandlerValue(value)
                    If Not String.IsNullOrEmpty(fieldOut) Then Call SetValueOutput(fieldOut, value)
                End If
                LogExecution("SetValueJson", StateLog.Finish)
                Return apiRest.SetValueJson(element, value, envelope)
            Catch ex As Exception
                Return Nothing
            End Try
        End Function

        Public Function CallAPIRest(uri As String, json As String, parametersApiRest As String, typeExecution As String) As APIRestLeanTest
            Try
                Dim apirest = New APIRest
                Dim apiRestLeanTest = New APIRestLeanTest
                LogExecution("Url: " & uri, StateLog.Start)
                LogExecution("CallAPIRest: " & json, StateLog.Information)
                apiRestLeanTest = apirest.CallAPIRest(uri, json, parametersApiRest, p_IDTest, p_TableTest, typeExecution)
                LogExecution("CallAPIRest: " & json, StateLog.Finish)
                Return apiRestLeanTest
            Catch ex As Exception
                Return Nothing
            End Try
        End Function
